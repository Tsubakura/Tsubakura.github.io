...
First, the tower spawn locations. The tower spawn locations are dependent on where the wing clones are looking at. If they're looking straight at each other, then it means a tower will spawn under the clone. If however, the clones are looking diagonally, then it means the towers will spawn at a different cardinal direction. The tower will always spawn opposite of each other, so its not possible for one clone to jump west, while the other jumps north/south.
