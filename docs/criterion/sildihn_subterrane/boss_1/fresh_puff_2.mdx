...
Four puffs, two ice and two lightning, will drop on each cardinal or intercardinal positions of the arena. Each of these puffs will randomly tether to one player and after a set time, the puffs will leap a set distance (two tiles) towards the direction of the player it is tethered to. Once the puffs have leaped, the boss will cast <strong>Soaping Spree</strong> to resolve its tails and all the puffs. Thus, your goal as a party is to move the puffs in a way that allows your party to dodge the attacks.

There are 8 possible combinations, but instead of trying to remember each and every one of the combinations, here are some very simple rules you can use to consistently resolve **all** of them:
...
- Ice puffs movements are determined by the element on Silkie's tail:
  - **Ice**: Follow the same rules as lightning, move it to the respective wall/corner.
  - **Wind**: Do not move it directly to its respective wall/corner, but you can do anything else instead as long as the ice does not hit the safe zone in the center.
...
<table>
    ...
    <tr>
        <td>A mistake was made by the other ice puff player. Can you spot it?</td>
        <td>The wind patterns are considerably easier.</td>
    </tr>
</table>

Once the puffs are resolved, Silkie will now either use <strong>Fizzling Suds</strong> or <strong>Bracing Suds</strong>, which is followed up by <strong>Slippery Soap</strong>. Identify which one Silkie casts, and resolve the stack and the underlying mechanic once you know whether its lightning or wind.

<table>
    <tr>
        <th>Fizzling Suds (lightning)</th>
        <th>Bracing Suds (wind)</th>
    </tr>
    <tr>
